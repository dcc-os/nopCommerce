        ...
        public ActionResult PDTHandler(FormCollection form)
        {
            ...
            if (processor.GetPdtDetails(tx, out values, out response))
            {
                ...
                if (order != null)
                {
                    ...
                    {
                        var errorStr = string.Format("PayPal PDT. Returned order total {0} doesn't equal order total {1}. Order# {2}.", mc_gross, order.OrderTotal, order.Id);
                        //log
                                    ...
                                    case PaymentStatus.Authorized:
                                        {
                                            ...
                                            {
                                                //not valid
                                                var errorStr = string.Format("PayPal IPN. Returned order total {0} doesn't equal order total {1}. Order# {2}.", mc_gross, order.OrderTotal, order.Id);
                                                //log
                                                _logger.Error(errorStr);
                                                //order note
                                                order.OrderNotes.Add(new OrderNote
